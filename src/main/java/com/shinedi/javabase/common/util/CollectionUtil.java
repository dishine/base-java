...
public class CollectionUtil {
    ...
    public static <T> List<List<T>> partCollection(List<T> list, int part) {
        int totalPart = list.size() % part == 0 ? list.size() / part : list.size() / part + 1;
        List<List<T>> result = new ArrayList<>();
        for (int i = 0; i < totalPart; i++) {
            if (i != totalPart - 1) {
                result.add(list.subList(i * part, (i + 1) * part));
            } else {
                result.add(list.subList(i * part, list.size()));
            }
        }
        return result;
    }
